...
# Space-separated incoming interfaces for policy routing. Leave empty to apply to all traffic.
RULE_IIF_LIST="br0"
...
To apply policy routing to multiple ingress interfaces, list them space-separated. For example, to include both `Bridge0` and `xfrms2`:

```shell
RULE_IIF_LIST="br0 xfrms2"
```

If you want route-veil to apply to all traffic, including traffic generated by the router itself, set `RULE_IIF_LIST` to an empty quoted value in `/opt/etc/route-veil/config`:

```shell
RULE_IIF_LIST=""
```

After changing `RULE_IIF_LIST`, run:
